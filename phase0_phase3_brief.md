...
1. Iterative filtering removed ~8,000 reviews beyond the initial threshold pass. Does the professor consider this level of data reduction acceptable, or does it raise concerns about sample size for downstream modelling?

2. User genre preference vectors are computed from positive reviews only, weighted by playtime. Is playtime-weighting a justifiable proxy for preference strength — or could high playtime on a disliked game (sunk cost) introduce noise?

3. The negative sampling generates 4 synthetic negatives per positive. Since these negatives are concentrated in the user's genre space (harder), does the 1:4 ratio still hold, or should fewer hard negatives be used compared to the random negative literature?

4. LOO exclusion on test aggregates only restricts to training interactions before the test timestamp. Should we additionally exclude other test interactions for the same user that occur earlier in the test window, since those labels are also technically unseen at prediction time?
...
1. The wide component uses 3 hand-crafted cross features chosen on domain intuition. Is there a more principled way to select which feature crosses belong in the wide component, or is domain judgement the accepted practice?

2. The deep component ingests NCF embeddings learned from interaction patterns and BERT embeddings learned from text — two very different representation spaces concatenated together. Does the model need any explicit alignment mechanism between them, or does the dense layer learn to handle the mixing implicitly?

3. The ranking evaluation uses 99 randomly sampled negatives per user — the standard NCF protocol. But our training negatives are genre-informed (hard), while the evaluation negatives are random (easy). Does this mismatch mean HR@10 overestimates real-world ranking quality?

4. The guideline asks: "What has been actually learned by your models from the training data?" — without feature importance analysis, how can we justify that the model is learning meaningful signals (e.g. genre preferences, community reception) rather than fitting noise?
